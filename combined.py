...
# <<< MODIFIED: Now returns a tuple (is_fallen, aspect_ratio) >>>
def check_for_fall(bbox: tuple, supabase_client: Client) -> Tuple[bool, float]:
    ...
    # This print statement can be commented out if the on-screen display is sufficient
    # print(
...
def draw_info_overlay(frame, fps, tracking_status, motor_action, fall_detection_enabled, is_in_fall_state, aspect_ratio):
    ...
    # Display FPS
    cv2.putText(frame, f"FPS: {fps:.1f}", (10, y_pos), font, font_scale, (255, 255, 255), thickness)
    y_pos += 22

    # Display Motor Status
    motor_text = f"Motor: {motor_action.replace('pulsing_', '').upper()}"
    cv2.putText(frame, motor_text, (10, y_pos), font, font_scale, (255, 255, 255), thickness)
    y_pos += 22
    
    # Display Tracking Status
    track_status_text = "ACTIVE" if tracking_status else "INACTIVE / SEARCHING"
    track_color = (0, 255, 0) if tracking_status else (0, 165, 255) # Green for active, Orange for inactive
    cv2.putText(frame, f"Tracking: {track_status_text}", (10, y_pos), font, font_scale, track_color, thickness)
    y_pos += 22
    
    # Display Fall Detection Configuration Status
    fall_config_text = "ENABLED" if fall_detection_enabled else "DISABLED"
    fall_config_color = (255, 255, 255) # White
    cv2.putText(frame, f"Fall Detection: {fall_config_text}", (10, y_pos), font, font_scale, fall_config_color, thickness)
    y_pos += 22

    # Display Current Fall State and Aspect Ratio
    fall_state_text = "FALLEN" if is_in_fall_state else "NORMAL"
    fall_state_color = (0, 0, 255) if is_in_fall_state else (0, 255, 0) # Red for fallen, Green for normal
...
# <<< MODIFIED: Now calculates FPS and calls the info overlay function >>>
def run_tracking_and_fall_detection(picam2: Picamera2, motors: MotorController, supabase_client: Client, roboflow_client: InferenceHTTPClient):
    global FALL_DETECTION_ENABLED, is_in_fallen_state # Added is_in_fallen_state to read its status
    
    tracking_active, tracker, bbox, frame_counter = False, None, None, 0
    last_status_check_time = time.monotonic()
    STATUS_CHECK_INTERVAL = 10
    
    # Variables for info overlay
    fps = 0.0
    ...
    try:
        while True:
            ...
            # --- Frame Capture and Redetection Logic (unchanged) ---
            ...
            if not tracking_active or force_redetection:
                ...
                if bodies:
                    ...
                    stop_alarm()
                    bbox_history.clear()
                    is_in_fallen_state = False # Reset state if person is lost
                    current_aspect_ratio = 0.0
                    print("[Detection] No bodies found, history cleared, state reset.")
            
            # --- Tracking and Fall Detection Logic (MODIFIED) ---
            if tracking_active:
                ...
                if success:
                    ...
                    if FALL_DETECTION_ENABLED:
                        # Now captures both fall status and aspect ratio
                        is_fallen, current_aspect_ratio = check_for_fall(bbox, supabase_client)
                    else:
                        stop_alarm()
                        current_aspect_ratio = 0.0 # Reset when disabled
                    ...
                    stop_alarm()
                    is_in_fallen_state = False # Reset state if tracking is lost
                    current_aspect_ratio = 0.0
            
            # --- FPS Calculation and Display (NEW) ---
